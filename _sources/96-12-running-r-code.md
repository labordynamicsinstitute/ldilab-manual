...
In essence, these instructions show how to deal with the three most common actions that replicators have to undertake when running R code:

 1. Making sure that paths (i.e., something like "`Mycomputer/Documents/Workspace/`") in the .do files (R scripts) reflect the appropriate location of code, data, and output in the computer where the code is run.
 ...
 3. Creating .log files (files that record, in this case, R output) of the replication attempts.
...
A master .do file is a R script that will call, in the correct sequence, all the programs necessary to construct analysis datasets, do all computations, and produce figures and tables. If a master do file exists, it should be mentioned in the README. In most cases, running a single master do file is sufficient to complete the reproduction. In general, a master script does not need to be a .do file. However, we will focus on cases where all work done in R is reduced to executing a single .do file.
...
A crucial function of `config.do` is that it allows for the local installation of R packages, which is important for two reasons. First, it will enable us to check for the completeness of replication materials. Second, when running code in servers, we often do not have the necessary permissions to install R packages freely.`config.do` allow us to installed packages in the replication directory.  
...
> - If the replication package includes a folder with R packages, add the line  `adopath ++` followed by the path of the location of that folder and save. See [Appendix F](https://labordynamicsinstitute.github.io/replicability-training-curriculum/using-config-do-in-stata.html) for details.
...
This option will set the working directory  to the location where the `master.do` is. It opens R and will show the processes in the R window.
...
On Unix-style systems, the preferred way is to use the command line to run R code.
...
Identify which R version you have installed (some systems have only one, some have multiple):
...
If a you find a bug that is simple enough to fix, you can make changes to the do files. Then,  you can right click on the master file and select `Execute (do)` as this option will open R, allowing to run the code interactively.
